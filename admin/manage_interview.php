...
$requestMode = $_POST['mode'] ?? ''; // Fixing overwriting issue

if ($requestMode === "schedule" || $requestMode === "edit") {
    $applicationId = $_POST['application_id'] ?? '';
    $date = $_POST['interview_date'] ?? '';
    $time = $_POST['interview_time'] ?? '';
    $interviewMode = $_POST['interview_mode'] ?? ''; // Avoid conflict with $mode
...
} elseif ($requestMode === "update_result") { // FIXED: Correct variable usage
    $applicationId = $_POST['application_id'] ?? '';
    ...
    $stmt = $conn->prepare("UPDATE interviews SET result = ? WHERE application_id = ?");
    $stmt->bind_param("si", $result, $applicationId);

    if ($stmt->execute()) {
        echo json_encode(["status" => "success", "message" => "Interview result updated"]);
    } else {
        error_log("Database Error: " . $stmt->error);
        echo json_encode(["status" => "error", "message" => "Database error occurred. Please try again."]);
    }
